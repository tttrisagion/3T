    ...
    with tracer.start_as_current_span("fetch_and_store_balance") as span:
        ...
        try:
            ...
            if not all([api_key, wallet_address, private_key]):
                ...
                if not wallet_address: missing.append("walletAddress")
                if not private_key: missing.append("privateKey")
                raise ValueError(f"Missing required Hyperliquid secrets in secrets.yml: {', '.join(missing)}")
            ...
            cursor.execute("SELECT id, symbol FROM products WHERE exchange_id = (SELECT id FROM exchanges WHERE name = 'HyperLiquid')")
            ...
            if len(exchange_status['info']['assetPositions']) > 0:
                for position_data in exchange_status['info']['assetPositions']:
                    ...
                    if symbol in products:
                        ...
                        unrealized_pnl = float(position_data['position']['unrealizedPnl'])

                        query = "INSERT INTO positions (product_id, position_size, position_value, unrealized_pnl) VALUES (%s, %s, %s, %s)"
                        cursor.execute(query, (product_id, position_size, position_value, unrealized_pnl))

            account_value = float(exchange_status['info']['marginSummary']['accountValue'])
            cross_maintenance_margin_used = exchange_status['info']['crossMaintenanceMarginUsed']
            ...
            cursor.execute(query, (exchange_id, account_value, cross_maintenance_margin_used))
