...
where $H_{\text{cur}}$ is the Kelly percentage from current runs and $H_{\text{hist}}$ is the aggregate Kelly percentage derived exclusively from \textbf{historically profitable market regimes} (filtered by block expectancy). This ``positive expectancy gate'' ensures the current strategy is compared against a robust ``winner's index'' rather than a global average that may be diluted by noise.

The factor $P_{\text{kelly}}$ is then clipped to a symmetric bound $[-\tau,\tau]$ (default $\tau = 0.5$) and floored at $-0.98$. The final position size is:
...
A key aspect of this module is its symbiotic relationship with the Parameter Optimization Engine. The engine's continuous simulations provide an intelligent, real-time benchmark of expected performance for thousands of alternative parameter configurations. 
