...
def get_price(symbol, max_retries=None):
    """
    Get price for a symbol from shared memory.
    Returns (price, timestamp) if found and fresh, otherwise retries.
    """
        ...
        try:
            ...
            try:
                ...
                for i in range(min(num_symbols, MAX_SYMBOLS)):
                    ...
                    stored_hash, price, timestamp = struct.unpack("<Qdd", entry_data)

                    if stored_hash == symbol_hash:
                        # Check staleness
                        age = current_time - timestamp
                        if age <= MAX_AGE_SECONDS:
                            return price, timestamp, age
                        else:
                            print(
                                f"Price for {symbol} is stale (age: {age:.1f}s), retrying..."
                            )
                            break
    ...
    raise TimeoutError(
        f"Could not get fresh price for {symbol} after {retry_count} attempts"
    )
...
def main():
    if len(sys.argv) < 2:
        ...
        print("Add --once to get price once and exit")
        sys.exit(1)

    symbol = sys.argv[1]
    once = "--once" in sys.argv

    print(f"Getting price for symbol: {symbol}")
    ...
    try:
        while True:
            try:
                price, timestamp, age = get_price(symbol)
