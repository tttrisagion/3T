...
WALLET_ADDRESSES = [
    address.strip() for address in wallet_addresses_str.split(",") if address.strip()
]
...
async def poll_positions_periodically():
    ...
    print(
        f"Observer: Starting background polling task for {len(WALLET_ADDRESSES)} addresses..."
    )
    ...
    if os.path.exists(OUTPUT_FILE):
        try:
            with open(OUTPUT_FILE) as f:
                ...
                print(
                    f"Observer: Loaded data for {len(data.get('positions', {}))} addresses from disk."
                )
        ...
        try:
            ...
            async with ccxt.hyperliquid(
                ...
                for address in WALLET_ADDRESSES:
                    try:
                        ...
                        if state:
                            ...
                            for dex in HIP3_DEXES:
                                try:
                                    ...
                                    if hip3_state:
                                        ...
                                        hip3_margin = hip3_state.get(
                                            "marginSummary", {}
                                        )
                                        account_value += float(
                                            hip3_margin.get("accountValue", 0)
                                        )
                                        total_margin_used += float(
                                            hip3_margin.get("totalMarginUsed", 0)
                                        )
                                    ...
                                    print(
                                        f"Observer: HIP-3 error for {address} on {dex}: {e}"
                                    )
                            ...
                            state["marginSummary"]["totalMarginUsed"] = str(
                                total_margin_used
                            )
                        ...
                        if "429" in str(e):
                            print(
                                "Observer: Rate limited (429). Skipping remaining addresses this cycle."
                            )
                            break
                ...
                if new_positions:
                    ...
                    print(
                        f"Observer: Cache updated with {len(new_positions)} addresses at {timestamp}"
                    )
