...
def providence_trading_iteration(self, run_id):
    ...
    if state is None:
        ...
        try:
            ...
            cursor.execute("SELECT run_state, exit_run, end_time FROM runs WHERE id = %s", (run_id,))
...
def _perform_trading_iteration(run_id, state):
    ...
    if volatility is None:
        return {"status": "continue", "run_id": run_id, "info": "volatility_unavailable"}
    ...
    metrics = voms.get_metrics()
    balance = metrics['account_balance'] if metrics else state['start_balance']
    apr = ((balance - state['start_balance']) / (duration / 3600) * 24 * 365.24 / state['start_balance']) if state['start_balance'] > 0 and duration > 0 else 0
    ...
    recent_aprs = [
        a for i, a in enumerate(state['aprs'])
        if (time.time() - state['aprs_timestamp'][i]) / 60 < ann_params['rolling_apr_minutes']
    ]
    ...
    if state['weights_timestamp'] and state['weights_timestamp'][0] < weights_cutoff:
        state['weights'] = [w for i, w in enumerate(state['weights']) if state['weights_timestamp'][i] >= weights_cutoff]
        state['weights_timestamp'] = [ts for ts in state['weights_timestamp'] if ts >= weights_cutoff]

    aprs_cutoff = now - (ann_params['rolling_apr_minutes'] + 1) * 60
    if state['aprs_timestamp'] and state['aprs_timestamp'][0] < aprs_cutoff:
        state['aprs'] = [a for i, a in enumerate(state['aprs']) if state['aprs_timestamp'][i] >= aprs_cutoff]
        state['aprs_timestamp'] = [ts for ts in state['aprs_timestamp'] if ts >= aprs_cutoff]

    # --- Check APR Target Exit ---
    volatility_goal = calculate_volatility_goal(volatility, instrument_price, ann_params['min_goal_weight'], ann_params['max_goal_weight'], ann_params['min_goal'], ann_params['max_goal'])
    ...
    if machine_vision_entropy < ann_params['machine_vision_entropy_max']:
        ...
        approve, new_side = should_approve_trade(state['position_direction'], state['apr_change'], side, ann_params['system_swing'])

        if approve:
            # Check reversal exit
            if state['position_direction'] == 0 and duration > ann_params['max_direction_reversal']:
            ...
            # Check margin ratio
            cross_margin_ratio = (metrics['margin_used'] / balance) if metrics and balance > 0 else 0
            if cross_margin_ratio > ann_params['max_cross_margin_ratio']:
                if (state['position_direction'] > 0 and new_side == "buy") or \
                   (state['position_direction'] < 0 and new_side == "sell"):
                    ...
                    return {"status": "continue", "run_id": run_id, "info": "margin_limit"}

            # Check trade conditions
            trade_condition = (
                (apr >= 0 and state['position_direction'] <= 0 and new_side == "sell") or
                (apr <= 0 and state['position_direction'] < 0 and new_side == "buy") or
                (apr >= 0 and state['position_direction'] >= 0 and new_side == "buy") or
            ...
            if trade_condition and abs(state['position_direction']) <= max_pos_limit:
                pos_size = (state['start_balance'] * ann_params['pos_weight'] / instrument_price * ann_params['pos_scaler'] / ann_params['balance_divisor'])
    ...
    try:
        ...
        if iteration_count % 100 == 0:  # Persist every 100 iterations
            ...
            try:
                ...
                current_pnl = 0
                if state.get('voms_state') and state.get('voms_state', {}).get('trades'):
                    from shared.voms import VOMS
                    ...
                    metrics = voms.get_metrics()
                    if metrics:
                        current_pnl = metrics['account_balance'] - state['start_balance']
                ...
                db_cnx.commit()
                logger.debug(f"Run {run_id}: Persisted state to MySQL at iteration {iteration_count} (pos: {state.get('position_direction', 0)}, pnl: {current_pnl:.2f})")
            except Exception as e:
...
def providence_supervisor(self):
    ...
    with tracer.start_as_current_span("providence_supervisor"):
        ...
        try:
            ...
            cursor.execute("SELECT id FROM runs WHERE end_time IS NULL AND exit_run = 0")
            ...
            if needed_runs > 0:
                ...
                for _ in range(needed_runs):
                    ann_params = generate_ann_params(symb, symb_leverage, virtual_balance)
                    choose = ann_params['choose']
                    ...
                    logger.info(f"Supervisor: Created run {cursor.lastrowid} for {symb[choose]}")
