...
fire_and_forget_app = Celery(
    "showcase_client_producer",
    broker="redis://192.168.2.157:6379/0",
    backend="redis://192.168.2.157:6379/0",
)
fire_and_forget_app.conf.broker_pool_limit = 100
        ...
        while True:
            ...
            try:
                result = await self._loop.run_in_executor(
                    self._executor, self._sync_get_result, task_id
                )
...
# --- Main Trading Logic ---
async def trading_task(result_fetcher, executor, resume_state=None):
    ...
    async def get_max_pos():
        task = await loop.run_in_executor(
            executor,
            fire_and_forget_app.send_task,
            "worker.tasks.get_active_run_count",
            [symb[choose]],
        )
    ...
    async def create_new_run():
        ...
        task = await loop.run_in_executor(
            executor,
            lambda: fire_and_forget_app.send_task(
                "worker.tasks.create_run", kwargs=run_params
            ),
        )
    ...
    async def update_run(balance):
        await loop.run_in_executor(
            executor,
            fire_and_forget_app.send_task,
            "worker.tasks.end_run",
            [run_id, balance],
        )

    async def update_run_position(pos):
        await loop.run_in_executor(
            executor,
            fire_and_forget_app.send_task,
        ...
        if os.path.exists(file_path):
            try:
                completed_path = os.path.join(
                    COMPLETED_DIR, f"run_{run_id}_completed_{int(time.time())}.json"
                )
    ...
    def get_voms_values():
        ...
        if metrics:
            ...
            return (0.0, 0.0, 0.0, virtual_balance, 0.0)
    ...
    try:
        if not resume_state:
            ...
            while attempts < 20:
                ...
                choose = random.randint(0, len(symb) - 1)
                ann_params["choose"] = choose
                voms = VOMS(
                    starting_balance=virtual_balance, leverage=symb_leverage[choose]
                )
                attempts += 1
                await asyncio.sleep(1)

            if attempts >= 20:
                print("Could not find open slot after 20 attempts. Exiting.")
                return
        ...
        if start_balance <= 0:
            logging.warning(
                f"Run {run_id} has invalid start_balance {start_balance}. Resetting."
            )
            start_balance = virtual_balance
        ...
        while True:
            ...
            await update_pnl(balance - virtual_balance)
            aprs.append(apr)
            aprs_timestamp.append(time.time())
            voms.update_price(instrument_price)

            # --- Memory Management: Prune old data to prevent unbounded growth ---
            now = time.time()

            # Prune weights based on its specific lookback window
            weights_cutoff = (
                now - (volatility_entropy_window_minutes + 1) * 60
            )  # Add 1 min buffer
            first_valid_weights_index = next(
                (i for i, ts in enumerate(weights_timestamp) if ts >= weights_cutoff),
                None,
            )
            if first_valid_weights_index is not None and first_valid_weights_index > 0:
                weights = weights[first_valid_weights_index:]
                weights_timestamp = weights_timestamp[first_valid_weights_index:]

            # Prune APRs based on its specific lookback window
            aprs_cutoff = now - (rolling_apr_minutes + 1) * 60  # Add 1 min buffer
            first_valid_aprs_index = next(
                (i for i, ts in enumerate(aprs_timestamp) if ts >= aprs_cutoff), None
            )
            if first_valid_aprs_index is not None and first_valid_aprs_index > 0:
                aprs = aprs[first_valid_aprs_index:]
                aprs_timestamp = aprs_timestamp[first_valid_aprs_index:]

            exit_trade = False
            if await get_exit():
                format_log("E", False, "I", "EXIT ON GLOBAL EXTERNAL EXIT SIGNAL")
                await asyncio.sleep(random.random() * 100)
                await loop.run_in_executor(None, cleanup_state)
                break
            elif apr > (
                float(apr_target)
                * float(
            ...
            if exit_trade:
                if get_position():
                    ...
                    format_log(
                        ...
                        False,
                        "I",
                        f"BOOK EXIT ORDER {exit_side}, {abs(position_size)}",
                    )
                    if (abs(position_size) * instrument_price) > 10:
                        await asyncio.sleep(DECISION_SLEEP)
                await update_run(balance)
            ...
            if machine_vision_entropy < machine_vision_entropy_max:
                try:
                    ...
                    if approve_trade:
                        if (
                            ...
                            if get_position():
                                ...
                                format_log(
                                    ...
                                    False,
                                    "I",
                                    f"BOOK EXIT ORDER {'sell' if p_size > 0 else 'buy'}, {abs(p_size)}, {instrument_price}",
                                )
                                if (abs(p_size) * instrument_price) > 10:
                                    await asyncio.sleep(DECISION_SLEEP)
                            await update_run(balance)
                            await loop.run_in_executor(None, cleanup_state)
                            break
                        ...
                        cross_margin_ratio = (
                            get_cross_maintenance_margin_used() / balance
                            if balance > 0
                            else 0
                        )
                        if not (
                            ...
                            if (
                                trade_condition
                                and abs(position_direction) <= max_pos_limit
                            ):
                                await asyncio.sleep(DECISION_SLEEP)
                                format_log(
                                    "E",
                                    False,
                                    "I",
                                    f"BOOK NORMAL ORDER {side}, {pos[choose]}, {instrument_price} based on {position_direction} {apr_change}",
                                )
                                position_direction += 1 if side == "buy" else -1
                                await update_run_position(position_direction)
                                voms.add_trade(
                                    pos[choose] * (1 if side == "buy" else -1)
                                )
                except Exception as e:
...
async def main():
    ...
    async def exit_runs_by_height(next_height):
        await loop.run_in_executor(
            executor,
            fire_and_forget_app.send_task,
            "worker.tasks.set_exit_for_runs_by_height",
            [next_height],
        )
    ...
    try:
        ...
        for state in saved_runs:
            ...
            task = asyncio.create_task(
                trading_task(result_fetcher, executor, resume_state=state)
            )
            tasks.add(task)

        remaining_slots = NUM_CHILDREN - len(tasks)
        for _ in range(remaining_slots):
            task = asyncio.create_task(trading_task(result_fetcher, executor))
            tasks.add(task)
        ...
        while tasks:
            ...
            for task in done:
                ...
                tasks.remove(task)
                new_task = asyncio.create_task(trading_task(result_fetcher, executor))
                tasks.add(new_task)
            ...
            if loop.time() % 60 < 1:
                try:
                    ...
                    if next_height > current_height:
                        print(
                            "MANAGER: TARGET HEIGHT REACHED, SHUTTING DOWN ALL TASKS."
                        )
                        for task in pending:
                            task.cancel()
                        await asyncio.gather(*pending, return_exceptions=True)
                        await exit_runs_by_height(current_height + 1)
                        break
                except Exception as e:
